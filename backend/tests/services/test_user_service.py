...
async def test_ensure_email_available_raises_on_duplicate() -> None:
    ...
    with patch("app.services.user_service.crud") as mock_crud:
        ...
        with pytest.raises(HTTPException) as exc_info:
            await _ensure_email_available(
                session=mock_session, email="taken@example.com"
            )
...
async def test_ensure_email_available_allows_same_user() -> None:
...
async def test_create_user_skips_email_when_disabled() -> None:
...
async def test_delete_user_me_rejects_superuser() -> None:
    ...
    with pytest.raises(HTTPException) as exc_info:
        await delete_user_me(session=mock_session, current_user=superuser)
